...
## Redesign do painel admin — Rodada 1 em produção (2026-09-11)

`psicomap-admin.html` passou por um redesign de design system em 7 commits (PR
[#65](https://github.com/elevaitconsultoria/pseg-safesign/pull/65), squash-mergeada em
`9ba04fe`): `:root` de 19 para ~110 tokens, sidebar/login neutros com acento verde, botões/
KPI/modal/tabela/badge unificados por agrupamento de seletor, estados de sistema padronizados
(`_skeleton`/`_loadingState`/`_emptyState`/`_errorState`), tela GHE e tela Links saindo do
layout 100% inline, e contexto único de empresa/ciclo com filtros auto-apply. `main` e
`develop` estão sincronizados (ambos em `9ba04fe`). Zero mudança em `.sql` ou
`psicomap-forms.html`. Detalhamento completo em
`.claude/notes/2026-09-11-redesign-painel-rodada1-producao.md`.

**Merge squash quebra a receita de sync `develop`↔`main` documentada acima.** A receita
"`git push origin origin/main:develop` (fast-forward)" só vale para merge sem squash. Com
squash, `main` ganha um commit que não é descendente do último commit de `develop` — o fast-
forward é rejeitado pelo git. Confirmar sempre com `git merge-base --is-ancestor
origin/develop origin/main` antes de escolher entre push simples e
`git push origin origin/main:develop --force` (seguro só depois de confirmar que o conteúdo
final é idêntico e que não há trabalho local não commitado).

**Bugs conhecidos, ainda não corrigidos** (plano de correção em
`C:\Users\consu\.claude\plans\quero-que-vc-atue-agile-newell.md`, PR #66 ainda não aberta):

- **Comparativo e Plano de Ação renderizam sem estilo nenhum** — usam 5 classes que nunca
  existiram no CSS (`.filter-grp`, `.filter-lbl`, `.filter-sel`, `.btn-prim`, `.btn-sec`).
  Pré-existente, não causado pelo redesign; ficou mais visível porque o resto do app mudou de
  visual ao redor.
- **`popularSelectsEmpresas()` não inclui `audit-empresa`, `agrup-empresa`, `ad-empresa` nem
  `ghe-empresa-sel`** — Auditoria (e as outras 3 telas) não populam o select de cliente na
  primeira entrada, só na segunda navegação. Pré-existente, confirmado por comparação lado a
  lado com a versão anterior ao redesign.
- **`--g-ring` (anel de foco) com contraste ~1,3:1** contra o mínimo de 3:1 — defeito
  introduzido pelo próprio redesign, achado em auditoria própria antes do merge.
- **Reportado pelo usuário, nunca reproduzido**: "Seções do laudo" não refletindo a seleção no
  preview em uso real. A lógica testada isoladamente e em modo demo (sem login) está correta;
  suspeita não confirmada de erro assíncrono não tratado em `renderLaudo()` com dados reais.
  Precisa de reprodução com sessão logada — ver `[[feedback-auth-impersonation-blocked]]`.
